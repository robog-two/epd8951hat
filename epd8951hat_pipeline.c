// SPDX-License-Identifier: GPL-2.0
/*
 * epd8951hat_pipeline.c – Pure pixel-pipeline transforms.
 *
 * Implements the three purely-computational stages of the display pipeline:
 *   1. Floyd-Steinberg XRGB8888 → 1bpp (epd_dither_xrgb8888_fn)
 *   2. Mirror + per-byte diff → dirty bounding box (epd_compute_dirty_rect)
 *   3. 4-byte alignment of the dirty byte range (epd_align_dirty_bytes)
 *   4. LUT variant classification (epd_lut_classify)
 *
 * This file is intentionally free of SPI, GPIO, mutex, or DRM calls so that
 * it can be compiled and tested in userspace (see tests/).
 */

#ifndef TESTING_BUILD
#  include <linux/bitrev.h>  /* bitrev8 */
#  include <linux/kernel.h>  /* clamp, min */
#  include <linux/slab.h>    /* kcalloc, kfree */
#  include <linux/string.h>  /* memset, strnstr */
#endif

#include "epd8951hat_pipeline.h"

/* =========================================================================
 * Stage 1 – Floyd-Steinberg dithering
 * ========================================================================= */

void epd_dither_xrgb8888_fn(u16 w, u16 h, u32 stride,
			      const u8 *src, u32 src_pitch,
			      u8 *mono_buf)
{
	int *err_cur, *err_nxt, *tmp;
	int x, y;

	memset(mono_buf, 0, (size_t)stride * h);

	err_cur = kcalloc(w + 2, sizeof(int), GFP_KERNEL);
	err_nxt = kcalloc(w + 2, sizeof(int), GFP_KERNEL);
	if (!err_cur || !err_nxt) {
		/* OOM fallback: simple threshold, no error diffusion */
		for (y = 0; y < (int)h; y++) {
			const u8 *row = src + (size_t)y * src_pitch;

			for (x = 0; x < (int)w; x++) {
				const u8 *px = row + (size_t)x * 4;
				int g = (77 * px[2] + 150 * px[1] + 29 * px[0]) >> 8;

				if (g < 128)
					mono_buf[(size_t)y * stride + x / 8] |=
						(u8)(0x80u >> (x & 7));
			}
		}
		goto out;
	}

	for (y = 0; y < (int)h; y++) {
		const u8 *row = src + (size_t)y * src_pitch;

		memset(err_nxt, 0, (w + 2) * sizeof(int));

		for (x = 0; x < (int)w; x++) {
			const u8 *px = row + (size_t)x * 4;
			int gray = (77 * px[2] + 150 * px[1] + 29 * px[0]) >> 8;
			int val  = clamp(gray + err_cur[x + 1] / 16, 0, 255);
			int new_val, err;

			if (val < 128) {
				new_val = 0;
				mono_buf[(size_t)y * stride + x / 8] |=
					(u8)(0x80u >> (x & 7));
			} else {
				new_val = 255;
			}

			err = val - new_val;
			/* 7/16 right, 3/16 down-left, 5/16 down, 1/16 down-right */
			err_cur[x + 2]        += err * 7;
			if (x > 0) err_nxt[x] += err * 3;
			err_nxt[x + 1]        += err * 5;
			err_nxt[x + 2]        += err;
		}

		tmp     = err_cur;
		err_cur = err_nxt;
		err_nxt = tmp;
	}

out:
	kfree(err_cur);
	kfree(err_nxt);
}

/* =========================================================================
 * Stage 2 – Mirror + dirty bounding box
 * ========================================================================= */

void epd_compute_dirty_rect(u16 h, u32 stride, bool mirror_x,
			      const u8 *mono_buf, u8 *flip_buf,
			      int *y0_out, int *y1_out,
			      int *b0_out, int *b1_out)
{
	/* Sentinels: y0 > y1 means "nothing dirty". */
	int y0 = (int)h,      y1 = -1;
	int b0 = (int)stride, b1 = -1;
	int y, b;

	for (y = 0; y < (int)h; y++) {
		for (b = 0; b < (int)stride; b++) {
			u8 nb;

			if (mirror_x)
				nb = bitrev8(mono_buf[(size_t)y * stride +
						      (stride - 1 - b)]);
			else
				nb = mono_buf[(size_t)y * stride + b];

			if (nb == flip_buf[(size_t)y * stride + b])
				continue;

			flip_buf[(size_t)y * stride + b] = nb;

			if (y < y0) y0 = y;
			if (y > y1) y1 = y;
			if (b < b0) b0 = b;
			if (b > b1) b1 = b;
		}
	}

	*y0_out = y0; *y1_out = y1;
	*b0_out = b0; *b1_out = b1;
}

/* =========================================================================
 * Stage 3 – 4-byte alignment
 * ========================================================================= */

void epd_align_dirty_bytes(u32 stride, bool needs_4byte_align,
			    int *b0, int *b1)
{
	if (!needs_4byte_align)
		return;

	*b0 &= ~3;
	*b1  = min(*b1 | 3, (int)stride - 1);
}

/* =========================================================================
 * LUT variant classification
 * ========================================================================= */

enum epd_lut_variant epd_lut_classify(const char *lut_str, size_t lut_bytes,
				       u8 *a2_mode, bool *needs_4byte_align)
{
	/* Priority matters: M841_TFA* must be checked before plain M841. */
	if (strnstr(lut_str, "M641", lut_bytes)) {
		*a2_mode           = EPD_MODE_A2_M641;
		*needs_4byte_align = true;
		return EPD_LUT_M641;
	}
	if (strnstr(lut_str, "M841_TFA2812", lut_bytes)) {
		*a2_mode           = EPD_MODE_A2_M841;
		*needs_4byte_align = false;
		return EPD_LUT_M841_TFA2812;
	}
	if (strnstr(lut_str, "M841_TFA5210", lut_bytes)) {
		*a2_mode           = EPD_MODE_A2_M841;
		*needs_4byte_align = false;
		return EPD_LUT_M841_TFA5210;
	}
	if (strnstr(lut_str, "M841", lut_bytes)) {
		*a2_mode           = EPD_MODE_A2_M841;
		*needs_4byte_align = false;
		return EPD_LUT_M841;
	}

	*a2_mode           = EPD_MODE_A2_M841;
	*needs_4byte_align = false;
	return EPD_LUT_UNKNOWN;
}
